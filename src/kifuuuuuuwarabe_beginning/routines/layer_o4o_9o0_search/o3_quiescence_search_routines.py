...
class O3QuiescenceSearchRoutines(SearchRoutines):
    ...
    def search_after_entry_node_quiescence(parent_pv, search_context_model):
        ...
        (remaining_moves, rolled_back) = O3QuiescenceSearchRoutines.filtering_same_destination_move_list(parent_move=parent_pv.vertical_list_of_move_pv[-1], remaining_moves=remaining_moves, rollback_if_empty=True) # できれば［同］の手を残す。
        remaining_moves = O3QuiescenceSearchRoutines.get_cheapest_move_list(remaining_moves=remaining_moves)
